...
////////////////
	...
	{
		//设置最大起飞速度
		s16 max_take_off_vel = LIMIT(My_Parame.set.auto_take_off_speed,20,200);
		//
		take_off_ok_cnt += dT_ms;
		auto_taking_off_speed = AUTO_TAKE_OFF_KP *(My_Parame.set.auto_take_off_height - wcz_hei_fus.out);
		//计算起飞速度
		auto_taking_off_speed = LIMIT(auto_taking_off_speed,0,max_take_off_vel);
		
		//退出起飞流程条件1，满足高度或者流程时间大于5000毫秒。
		if(take_off_ok_cnt>=5000 || (My_Parame.set.auto_take_off_height - loc_ctrl_2.exp[Z] <2))//(auto_ref_height>AUTO_TAKE_OFF_HEIGHT)
		{
...
////////////
	...
	{
		//设置自动下降速度
		auto_taking_off_speed = -(s16)LIMIT(My_Parame.set.auto_landing_speed,20,200);
...
{
	alt_arg_2.kp = My_Parame.set.pid_alt_2level[KP];
	alt_arg_2.ki = My_Parame.set.pid_alt_2level[KI];
	alt_arg_2.kd_ex = 0.00f;
	alt_arg_2.kd_fb = My_Parame.set.pid_alt_2level[KD];
	alt_arg_2.k_ff = 0.0f;
...
{
	alt_arg_1.kp = My_Parame.set.pid_alt_1level[KP];
	alt_arg_1.ki = My_Parame.set.pid_alt_1level[KI];
	alt_arg_1.kd_ex = 0.00f;
	alt_arg_1.kd_fb = 0;//My_Parame.set.pid_alt_1level[KD];
	alt_arg_1.k_ff = 0.0f;
...
void Alt_1level_Ctrl(float dT_s)
{
	...
	loc_ctrl_1.fb[Z] = wcz_spe_fus.out + My_Parame.set.pid_alt_1level[KD] *w_acc_z_lpf;//微分先行，下边PID函数微分系数为0
