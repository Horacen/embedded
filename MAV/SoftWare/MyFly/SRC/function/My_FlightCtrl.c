...
void Flight_State_Task(u8 dT_ms,s16 *CH_N)
{
	...
	/*设置油门摇杆量*/
	fs.speed_set_h_norm[Z] = (float)remoter.speed_hight;
	...
	/*速度设定量，正负参考My坐标参考方向*/
	fs.speed_set_h_norm[X] = (float)(remoter.angle_pitch / 27.0f);
	fs.speed_set_h_norm[Y] = (float)(-remoter.angle_rol / 27.0f);
	...
	speed_set_tmp[X] = fc_stv.vel_limit_xy *fs.speed_set_h_norm_lpf[X] + program_ctrl.vel_cmps_h[X];
	speed_set_tmp[Y] = fc_stv.vel_limit_xy *fs.speed_set_h_norm_lpf[Y] + program_ctrl.vel_cmps_h[Y];
...
///////////////////////////////////////////////////////
	...
	////==由高度和按键确定模式切换，3m以上为定高模式，3m下为光流模式
	static u16 cnt_mode;
	if(wcz_hei_fus.out < 300)
	{
		if(remoter.mode == 1)
		{
			cnt_mode += dT_ms;
