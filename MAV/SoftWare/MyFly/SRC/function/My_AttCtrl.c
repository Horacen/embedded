...
{
	arg_2[ROL].kp = My_Parame.set.pid_att_2level[ROL][KP];
	arg_2[ROL].ki = My_Parame.set.pid_att_2level[ROL][KI];
	arg_2[ROL].kd_ex = 0;//My_Parame.set.pid_att_2level[ROL][KD];
	arg_2[ROL].kd_fb = My_Parame.set.pid_att_2level[ROL][KD];
	arg_2[ROL].k_ff = 0.0f;
	
	arg_2[PIT].kp = My_Parame.set.pid_att_2level[PIT][KP];
	arg_2[PIT].ki = My_Parame.set.pid_att_2level[PIT][KI];
	arg_2[PIT].kd_ex = 0;//My_Parame.set.pid_att_2level[PIT][KD];
	arg_2[PIT].kd_fb = My_Parame.set.pid_att_2level[PIT][KD];
	arg_2[PIT].k_ff = 0.0f;

	arg_2[YAW].kp = My_Parame.set.pid_att_2level[YAW][KP];
	arg_2[YAW].ki = My_Parame.set.pid_att_2level[YAW][KI];
	arg_2[YAW].kd_ex = My_Parame.set.pid_att_2level[YAW][KD];
	arg_2[YAW].kd_fb = My_Parame.set.pid_att_2level[YAW][KD];
	arg_2[YAW].k_ff = 0.0f;		
}
...
{
	arg_1[ROL].kp = My_Parame.set.pid_att_1level[ROL][KP];
	arg_1[ROL].ki = My_Parame.set.pid_att_1level[ROL][KI];
	arg_1[ROL].kd_ex = //0.000f   ;
	arg_1[ROL].kd_fb = My_Parame.set.pid_att_1level[ROL][KD];
	arg_1[ROL].k_ff = 0.0f;
	
	arg_1[PIT].kp = My_Parame.set.pid_att_1level[PIT][KP];
	arg_1[PIT].ki = My_Parame.set.pid_att_1level[PIT][KI];
	arg_1[PIT].kd_ex = //0.000f   ;
	arg_1[PIT].kd_fb = My_Parame.set.pid_att_1level[PIT][KD];
	arg_1[PIT].k_ff = 0.0f;

	arg_1[YAW].kp = My_Parame.set.pid_att_1level[YAW][KP];
	arg_1[YAW].ki = My_Parame.set.pid_att_1level[YAW][KI];
	arg_1[YAW].kd_ex = //0.00f   ;
	arg_1[YAW].kd_fb = My_Parame.set.pid_att_1level[YAW][KD];
...
void Att_2level_Ctrl(float dT_s,s16 *CH_N)
{
	...
	/*摇杆量转换为YAW期望角速度*/
	set_yaw_av_tmp = (s32)(remoter.speed_yaw);
